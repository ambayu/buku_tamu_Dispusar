...
<body>
    <div class="container" style="max-width: 90%; margin-bottom:100px; margin-top:100px;">
        ...
        <table id="data_kunjungan" class="display table table-bordered">
            ...
            <tbody>
                <?php $sqlkunjungan = mysqli_query($koneksi, "SELECT * FROM tb_pesanbuku");
                while ($datakunjungan = mysqli_fetch_assoc($sqlkunjungan)) { ?>
                    <tr>
                        <td><?= $no++; ?></td>
                        <td><?= date('d-m-Y', strtotime($datakunjungan['tanggal'])); ?></td>
                        <td><?= $datakunjungan['judul']; ?></td>
                        <td><?= $datakunjungan['penulis']; ?></td>
                        <td><?= $datakunjungan['penerbit']; ?></td>
                        <td><a class="btn btn-danger" href="aksipesan.php?aksi=hapus&id=<?= $datakunjungan['id']; ?>"><i class="bi bi-trash"></i></a></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
